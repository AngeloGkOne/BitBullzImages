...
async function manageFiles(directory) {
  try {
    ...
    const filesToKeep = {
      Rare: ['13_BBTap_Rare.png', '14_BBTap_Rare.png', '23_BBTap_Rare.png', '4_BBTap_Rare.png'], // Keep these 4 in this order
      Epic: ['24_BBTap_Epic.png', '22_BBTap_Epic.png'], // Keep these 2 in this order
    ...
    for (const rarity in filesToKeep) {
      ...
      // Keep files in the order specified in filesToKeep
      let filesToKeepInGroup = specifiedFiles.filter(file => filesInGroup.includes(file));
      ...
      if (filesToKeepInGroup.length < limits[rarity]) {
        const additionalFiles = filesInGroup
          ...
          .sort((a, b) => parseInt(a.split('_')[0]) - parseInt(b.split('_')[0])) // Sort numerically
    ...
    const commonFiles = fileGroups.Common
      .filter(file => {
        const prefix = parseInt(file.split('_')[0]);
    ...
    // Step 5: Rename files within each rarity, preserving order
    for (const rarity in filesByRarity) {
      const filesInGroup = filesByRarity[rarity];
      // For Rare, Epic, Legendary: Preserve the order from filesToKeep
      // For Common: Already sorted numerically
      if (rarity !== 'Common') {
        const specifiedOrder = filesToKeep[rarity] || [];
